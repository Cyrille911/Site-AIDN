"""
Modèles centraux du SaaS multi-tenant.
Isolent les données par organisation et permettent l'accès multi-domaines.
"""
...
class Organization(models.Model):
    ...
    status = models.CharField(
        _('Statut'), max_length=20, choices=Status.choices, default=Status.ACTIVE
    )
    branding = models.JSONField(
        _('Configuration visuelle'), default=dict, blank=True,
        help_text=_('Logo, couleurs, nom affiché, etc.')
    )
...
class Domain(models.Model):
    """Nom de domaine pointant vers une organisation."""
    ...
    domain = models.CharField(
        _('Domaine'), max_length=253, unique=True, db_index=True,
        help_text=_('Exemple : dpse.mfb.gouv.ci ou app.aidn.ci')
    )
    is_primary = models.BooleanField(
        _('Domaine principal'), default=False,
        help_text=_('Domaine principal utilisé pour les liens et emails')
    )
    is_active = models.BooleanField(_('Actif'), default=True)
    created_at = models.DateTimeField(_('Créé le'), auto_now_add=True)
...
class OrganizationUser(models.Model):
    """Membre d'une organisation avec un rôle SaaS."""

    class Role(models.TextChoices):
        ADMIN = 'admin_org', _('Administrateur organisation')
        EXPERT = 'expert', _('Expert')
        READER = 'reader', _('Utilisateur lecture')
        TRAINER = 'trainer', _('Formateur')

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE,
        related_name='organization_memberships', verbose_name=_('Utilisateur')
    )
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE,
        related_name='organization_memberships', verbose_name=_('Organisation')
    )
    role = models.CharField(
        _('Rôle SaaS'), max_length=20, choices=Role.choices, default=Role.READER
    )
    is_admin = models.BooleanField(
        _('Admin organisation'), default=False,
        help_text=_('Donne les mêmes droits que le rôle admin_org')
    )
    job_title = models.CharField(_('Fonction'), max_length=100, blank=True)
    joined_at = models.DateTimeField(_('Ajouté le'), auto_now_add=True)
    updated_at = models.DateTimeField(_('Modifié le'), auto_now=True)
    ...
    def clean(self):
        if self.is_admin and self.role != self.Role.ADMIN:
            self.role = self.Role.ADMIN

    def has_role(self, *roles):
        return self.role in roles or (self.is_admin and 'admin_org' in roles)


class TenantModel(models.Model):
    """
    Mixin abstrait pour isoler un modèle par organisation.
    À hériter en premier dans la liste des classes parentes.
    """
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE,
        related_name='%(class)ss', verbose_name=_('Organisation')
    )
...
# === Academy models ===
"""
Module Academy - structure vide pour formation et certification.

Ce module permettra à terme de gérer des parcours de formation,
des certifications et des contenus pédagogiques par organisation SaaS.
"""
...
class TrainingCourse(TenantModel, models.Model):
    """Cours/formation proposé au sein d'une organisation."""
...
class Certification(TenantModel, models.Model):
    """Certification délivrée au sein d'une organisation."""
    course = models.ForeignKey(
        TrainingCourse, on_delete=models.CASCADE, related_name='certifications'
    )
...
class StrategicPlan(TenantModel, models.Model):
    """Plan stratégique global d'une organisation (GAR)."""
    title = models.CharField(max_length=500)
    description = models.TextField(blank=True)
...
class StrategicAxis(TenantModel, models.Model):
    """Axe stratégique (Gouvernance, Attributs ou Résultats)."""
    plan = models.ForeignKey(
        StrategicPlan, on_delete=models.CASCADE, related_name='axes'
    )
    name = models.CharField(max_length=255)
    ...
    def __str__(self):
        return f"{self.axis_type} - {self.name}"


# === Monitoring (old planning) models ===
...
class PlanAction(TenantModel, models.Model):
    ...
    def save(self, *args, **kwargs):
        # Multi-tenant : assignation automatique de l'organisation courante
        if not self.organization_id:
            from planning.middleware import get_current_organization
...
# Modèle PlanActionMember (liaison User <-> PlanAction via invitation)
class PlanActionMember(models.Model):
    """Associe un utilisateur invité à un plan d'action avec un rôle."""

    class Role(models.TextChoices):
        POINT_FOCAL = 'point_focal', _('Point Focal')
        RESPONSABLE = 'responsable', _('Responsable')
        PARTICIPANT = 'participant', _('Participant')

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE,
        related_name='plan_memberships', verbose_name=_('Utilisateur')
    )
    plan_action = models.ForeignKey(
        PlanAction, on_delete=models.CASCADE,
        related_name='members', verbose_name=_('Plan d\'action')
    )
    role = models.CharField(
        _('Rôle dans le PA'), max_length=20, choices=Role.choices, default=Role.PARTICIPANT
    )
    invited_at = models.DateTimeField(_('Invité le'), auto_now_add=True)

    class Meta:
        verbose_name = _('Membre du plan d\'action')
        verbose_name_plural = _('Membres des plans d\'action')
        unique_together = [['user', 'plan_action']]

    def __str__(self):
        return f"{self.user.email} - {self.plan_action.reference} ({self.role})"


# Modèle Effet (inchangé)
class Effet(TenantModel, models.Model):
    id = models.AutoField(primary_key=True)
    ...
    def save(self, *args, **kwargs):
        if not self.organization_id and self.plan_id:
            self.organization = self.plan.organization
...
class Produit(TenantModel, models.Model):
    id = models.AutoField(primary_key=True)
    ...
    def save(self, *args, **kwargs):
        if not self.organization_id and self.effet_id:
            self.organization = self.effet.plan.organization
...
class Action(TenantModel, models.Model):
    id = models.AutoField(primary_key=True)
    ...
    def save(self, *args, **kwargs):
        if not self.organization_id and self.produit_id:
            self.organization = self.produit.effet.plan.organization
...
class Activite(TenantModel, models.Model):
    id = models.AutoField(primary_key=True)
    reference = models.CharField(max_length=50, blank=True)
    titre = models.CharField(max_length=500)
    ...
    def save(self, *args, **kwargs):
        if not self.organization_id and self.action_id:
            self.organization = self.action.produit.effet.plan.organization
    ...
    @staticmethod
    def check_activity_alerts(organization=None):
        ...
        suiveurs_qs = User.objects.filter(is_active=True, groups__name='SuiveurEvaluateur')
        activites_qs = Activite.objects.select_related('action__produit__effet__plan')
        if organization:
            suiveurs_qs = suiveurs_qs.filter(organization=organization)
            activites_qs = activites_qs.filter(organization=organization)
        suiveurs = suiveurs_qs
...
class ActiviteLog(TenantModel, models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.organization_id and self.activite_id:
            self.organization = self.activite.organization
        super().save(*args, **kwargs)
