...
def create_primary_domain_if_missing(sender, instance, created, **kwargs):
    """Crée automatiquement un domaine par défaut si l'organisation n'en a aucun."""
    if created and not instance.domains.exists():
        Domain.objects.create(
            organization=instance,
            domain=f"{instance.slug}.aidn.app",
            is_primary=True
        )
