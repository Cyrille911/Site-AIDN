...
from .models import PlanAction, Effet, Produit, Action, Activite, ActiviteLog, PlanActionMember
...
def get_pa_role(user, plan_action):
    """Retourne le rôle d'un utilisateur dans un plan d'action donné.
    Les suiveurs évaluateurs (rôle global) ont toujours le rôle 'suiveur_evaluateur'.
    Les autres obtiennent leur rôle via PlanActionMember, ou None."""
    if user.role == 'suiveur_evaluateur':
        return 'suiveur_evaluateur'
    membership = PlanActionMember.objects.filter(user=user, plan_action=plan_action).first()
...
def strategy_index(request):
    """Vue temporaire du module stratégie."""
    organization = getattr(request, 'organization', None)
...
def add_plan_action(request):
    ...
    organization = getattr(request, 'organization', None)
    if not organization:
        return render(request, 'planning/access_denied.html', {'message': 'Aucune organisation associée.'})

    if not request.user.is_authenticated:
        return render(request, 'planning/access_denied.html')

    if request.user.role != 'suiveur_evaluateur':
        return render(request, 'planning/access_denied.html', {'message': 'Seul un Suiveur Évaluateur peut créer un plan d\'action.'})
...
def edit_plan_action(request, id):
    ...
    if not organization:
        return render(request, 'planning/access_denied.html', {'message': 'Aucune organisation associée.'})
    plan_action = get_object_or_404(PlanAction, id=id, organization=organization)
    
    if not request.user.is_authenticated:
        return render(request, 'planning/access_denied.html')
    
    # Seuls les suiveurs évaluateurs (rôle global) peuvent modifier un PA
    if request.user.role != 'suiveur_evaluateur':
        return render(request, 'planning/access_denied.html', {'message': 'Vous n\'êtes pas suiveur évaluateur.'})
...
def plan_action_list(request):
    ...
    # Les suiveurs évaluateurs voient tous les PA de l'organisation
    # Les autres ne voient que ceux auxquels ils sont invités via PlanActionMember
    is_se = request.user.role == 'suiveur_evaluateur'
    if is_se:
        plans = PlanAction.objects.filter(organization=organization)
    else:
...
def plan_action_detail(request, id):
    ...
    if not organization:
        return render(request, 'planning/access_denied.html', {'message': 'Aucune organisation associée.'})
    plan = get_object_or_404(PlanAction, id=id, organization=organization)
    ...
    pa_role = get_pa_role(request.user, plan) if request.user.is_authenticated else None
...
def pao_list(request, plan_id):
    ...
    if not organization:
        return render(request, 'planning/access_denied.html', {'message': 'Aucune organisation associée.'})
    plan = get_object_or_404(PlanAction, id=plan_id, organization=organization)
...
def track_execution_detail(request, plan_id):
    ...
    # Vérification : seul un suiveur évaluateur (rôle global) peut accéder
    is_se = request.user.role == 'suiveur_evaluateur'
    if not is_se:
        return render(request, 'planning/access_denied.html', {'plan': plan})
    ...
    if request.method == 'POST':
        ...
        if 'update_pao_status' in data:
            ...
            return render(request, 'planning/track_execution_detail.html', {
                ...
                'activites_by_entity': {},
                'is_se': is_se,
                'pa_role': get_pa_role(request.user, plan),
            })
...
def operational_plan_matrix(request, plan_id, annee):
    ...
    context['pa_role'] = get_pa_role(request.user, plan) if request.user.is_authenticated else None
...
def invite_to_plan_action(request, plan_id):
    """Invite un utilisateur par email à participer à un plan d'action."""
    organization = getattr(request, 'organization', None)
    if not organization:
        return JsonResponse({'success': False, 'error': 'Aucune organisation associée.'}, status=403)

    if not request.user.is_authenticated:
        return JsonResponse({'success': False, 'error': 'Non authentifié.'}, status=401)

    plan_action = get_object_or_404(PlanAction, id=plan_id, organization=organization)

    # Seul un suiveur évaluateur (rôle global) peut inviter
    if request.user.role != 'suiveur_evaluateur':
        return JsonResponse({'success': False, 'error': 'Seul un suiveur évaluateur peut inviter des participants.'}, status=403)
    ...
    if request.method == 'POST':
        ...
        invite_role = data.get('role', 'point_focal')  # Role dans le PA
        if not email:
            return JsonResponse({'success': False, 'error': 'L\'email est requis.'}, status=400)

        # Valider le rôle
        valid_roles = [r[0] for r in PlanActionMember.Role.choices]
        if invite_role not in valid_roles:
            invite_role = PlanActionMember.Role.PARTICIPANT
        ...
        if created:
            ...
            user.save()
            logger.info(f"Nouvel utilisateur créé par invitation : {email}")

        # Associer au plan d'action (si pas déjà membre)
        member, member_created = PlanActionMember.objects.get_or_create(
            user=user,
        ...
        if not member_created:
            return JsonResponse({'success': False, 'error': f'{email} est déjà membre de ce plan d\'action.'}, status=400)

        # Envoyer un email d'invitation
        try:
            from django.core.mail import send_mail
            subject = f"Invitation à participer au plan d'action : {plan_action.titre}"
            message = (
                f"Bonjour,\n\n"
                f"Vous avez été invité(e) par {request.user.first_name} {request.user.last_name} "
                f"à participer au plan d'action \"{plan_action.titre}\" (Réf: {plan_action.reference}).\n\n"
                f"Connectez-vous à la plateforme AIDN pour y accéder.\n\n"
                f"Cordialement,\nL'équipe AIDN"
            )
            send_mail(subject, message, 'aidn.tech@gmail.com', [email], fail_silently=True)
        except Exception as e:
            logger.warning(f"Erreur envoi email invitation : {e}")

        return JsonResponse({
            'success': True,
            'message': f'{email} a été ajouté au plan d\'action.',
            'user_created': created,
            'user_name': f'{user.first_name} {user.last_name}',
        })

    # GET : retourner la liste des membres du PA
    members = PlanActionMember.objects.filter(plan_action=plan_action).select_related('user')
    members_data = [{
        'email': m.user.email,
        'name': f'{m.user.first_name} {m.user.last_name}',
        'role': m.get_role_display(),
        'invited_at': m.invited_at.strftime('%d/%m/%Y'),
    } for m in members]
